...
class PartiallyLabeled(Dataset):
    ...
    def __init__(self, dataset, all_idx, labeled_percent):
        ...
        labeled_idx = np.random.permutation(all_idx)[:self.n_labeled_data]
        self._is_labeled[labeled_idx] = 1
...
adapted from https://github.com/sbarratt/inception-score-pytorch
"""
...
def compute_inception_score(imgs, inception_model=None, cuda=True, batch_size=32, resize=False, splits=1):
...
def get_activations(images, model, batch_size=50, dims=2048, device='cpu'):
    ...
    # ds = ImagesDataset(files, transforms=TF.ToTensor())
...
def compute_fid(images, model, device, batch_size=32, dims=2048):
